package com.techyasoft.blureffect.ui.base;
...
public class RenderView extends SurfaceView implements SurfaceHolder.Callback {
    ...
    private static final int NUM_OF_TASKS_TO_RUN_PER_REFRESH = CPU_COUNT;
    ...
    class RenderThread extends Thread {
        ...
        private void doDraw(Canvas canvas) throws InterruptedException, ExecutionException {
            ...
            if (mDrawingLayerBitmap != null) {
                ...
                int counter = 0;
                while (!mDrawingQueue.isEmpty() && counter < NUM_OF_TASKS_TO_RUN_PER_REFRESH) {
                    try {
     ...
     * @param callable
     */
    protected void addTask(Callable<Collection<Pair<Rect, Bitmap>>> callable) {
    ...
    public void cancelAllTasks() {
        for (Future<Collection<Pair<Rect, Bitmap>>> future : mDrawingQueue) {
            future.cancel(true);
        }
        mDrawingQueue.clear();
    }

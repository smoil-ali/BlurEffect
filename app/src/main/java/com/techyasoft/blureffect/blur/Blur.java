...
public abstract class Blur {
    ...
    abstract void ready(Bitmap blurBitmap);

    public Blur(Bitmap source,int radius) {
        new Thread(() -> start(source, radius)).start();
    }



    private void start(Bitmap source,int radius){
        ...
        Log.d(TAG, "Blur: "+width+" "+height);
        if (Helper.hasZero(width, height)) {
            Log.d(TAG, "start: zero");
            return;
        }
    ...
    private void stack(Bitmap sentBitmap,int radius,boolean canReuseInBitmap){
        Log.d(TAG, "stack: called with radius "+radius);
        Bitmap bitmap;
        if (canReuseInBitmap) {
            bitmap = sentBitmap;
        } else {
